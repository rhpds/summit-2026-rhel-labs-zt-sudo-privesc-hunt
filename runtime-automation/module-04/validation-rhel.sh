...
if [ -f /var/log/messages ]; then
    if ! ls -l /var/log/messages | grep -q "^-...r....." > /dev/null 2>&1; then
        echo "FAIL: /var/log/messages not readable by group"
        exit 1
    fi
fi
...
if sudo -l -U devuser 2>/dev/null | grep -q "NOPASSWD"; then
    echo "FAIL: devuser still has sudo privileges"
    exit 1
fi

echo "PASS: Remediation complete - vulnerability fixed, access properly granted"
exit 0
